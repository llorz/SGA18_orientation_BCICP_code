function [] = texture_transfer_pMap(T12, S1, S2, B1, B2, save_path)
B2_inv = pinv(B2);
B1_inv = pinv(B1);
% T12: maps S1 -> S2, n1-dim vector
n1 = length(S1.surface.X);
n2 =  length(S2.surface.X);
P21 = sparse(1:n1,T12, ones(1,n2), n1, n2); % n1-by-n2 matrix, maps S2 -> S1
...
S2_vt = MESH_IO.generate_tex_coords(X2(:,ic), cols(1), cols(2), 1);
